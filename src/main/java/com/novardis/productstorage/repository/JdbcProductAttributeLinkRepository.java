...
public class JdbcProductAttributeLinkRepository implements ProductAttributeLinkRepository {
    ...
    public Long save(ProductAttributeLinkDto productAttributeLinkDto) {
        String sqlQuery = "insert into product_attribute_link (attribute_value_id, product_id, attribute_dic_id) values(?, ?, ?)";
        KeyHolder keyHolder = new GeneratedKeyHolder();
        jdbcTemplate.update(connection -> {
            PreparedStatement stmt = connection.prepareStatement(sqlQuery, new String[]{"id"});
            stmt.setLong(1, productAttributeLinkDto.getAttributeValueId());
            stmt.setLong(2, productAttributeLinkDto.getProductId());
            stmt.setLong(3, productAttributeLinkDto.getAttributeDicId());
            return stmt;
